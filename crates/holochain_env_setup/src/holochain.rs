...
pub fn spawn_holochain(
    ...
    let mut holochain = kill_on_drop(
        Command::new("holochain")
            ...
            .arg("--piped")
            .env("RUST_LOG", "debug")
            .env("HOLOCHAIN_INFLUXIVE_EXTERNAL", "1")
            .env("HOLOCHAIN_INFLUXIVE_EXTERNAL_HOST", "http://localhost:8086")
            .env("HOLOCHAIN_INFLUXIVE_EXTERNAL_BUCKET", "test-holochain")
            .env("HOLOCHAIN_INFLUXIVE_EXTERNAL_TOKEN", "gMH_gIHXqOLZn2OlH9XbOUwEZStmYL4VVP3jb0kSqdo0EFmc-TfvwPOlEVyfg8i5Cq51rBKjSnthTb-f_Uq98w==")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
